...
We first considered serial optimisations by using various profiling tools to minimise instruction count and by {\color{red} exploiting locality of some of the data.}
Next, we exploited the {\color{red} inherent} parallelism using the OpenCL programming language on the \textbf{NVIDIA GeForce 570 (GF110 architecture)}, available through the computer lab on the ``Graphic01'' machine.
...
\end{changemargin}
\end{figure}
As expected the program does not tax the memory system, given that the only updates are to two floating point values in the form of coordinate updates.
The profiler output for CPU utilisation is presented in Figure \ref{fig:cpu_util}
\begin{figure}[!h]
\begin{changemargin}{-20mm}{-20mm}
\center
...
{\color{red} Is this true? What about reads? Spatial/Temporal locality?}
...
However, we will tackle this challenge on the GPU by passing {\color{red} vectorised arrays} to the device.
...
Next, the {\color{red} branching code} for the variable \verb+f = min(l/3.0, 3.0/l)+ was modified to eliminate the costly division operation:
\begin{lstlisting}[ ]
f = l < 3.0 ? l / 3.0 : 3.0 / l;
\end{lstlisting}
Because the following calculations rely on the evaluation of this branch, and the number of {\color{red} mis-predictions} is large, this was an important step in speeding up the serial algorithm.
...
While the grids we studied exhibited quick convergence, there is no guarantee that this will be the case for other grids.

We looked at speeding up the \verb+element_area+ function call.
Inside the \verb+element_quality+ function, the call to \verb+element_area+ {\color{red} retrieves the same data elements as its calling function}.
However, combining them showed no effect, since the compiler already makes this optimisation in the code generation step.

{\color{red} The code spent some time accessing}
...
Although this saw a factor 2 speedup and produced almost identical results to the serial version, {\color{red} TODO: FIx eng: it opens the door for the possibility of adjacent Nodes being altered} (or worse, the same Node{\color{red}??}) by parallel threads of execution, leaving a lower quality grid than previously found.
...
OpenCL is a GPU computing framework supported by the open source community supported by most commerical GPU and CPU hardware.
